...
public class Input {
  ...
  public static void main(String[] args) throws FileNotFoundException {
    String[] fileNames = {"a_example.txt", "b_read_on.txt", "c_incunabula.txt",
        "d_tough_choices.txt", "e_so_many_books.txt", "f_libraries_of_the_world.txt"};
    for (int k = 0; k < fileNames.length; k++) {


      File fileIn = new File(fileNames[k]);
      File fileOut = new File("outputs/" + fileNames[k]);
      ...
      // Line one in a section - number of books in the library [space] number of days it takes to
      // set
      // it up [space]
      ...
      for (int i = 0; i < Integer.parseInt(firstLine[1]); i++) {
        ...
        Library tempLibrary = new Library(Integer.parseInt(libraryFirstLine[0]),
            Integer.parseInt(libraryFirstLine[1]), Integer.parseInt(libraryFirstLine[2]),
            bookScore);
      ...
      // bookScore is an array with the scores of all the books in it, get a score by bookScore[id
      // of
      // book]
      ...
      while (time - timeUsed > 0) {
        ...
        for (int i = 0; i < bts; i++) {
          if (i < librariesUsed.get(librariesUsed.size() - 1).numBooks) {
            librariesUsed.get(librariesUsed.size() - 1)
                .addBookToScan(librariesUsed.get(librariesUsed.size() - 1).booksIDS[i]);
          }
